...
export function getStatus(p: HouseholdProgress): Status {
  ...
  if (p.provided + p.pending_buyouts >= p.required && p.pending_buyouts > 0) return "pending";
...
export const statusStyles: Record<Status, { badge: string; card: string; label: string }> = {
  fulfilled: {
